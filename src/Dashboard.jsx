    ...
    try {
      ...
      const base64UrlDecode = (str) => {
        ...
        try {
          ...
          // If JSON parse fails, return the raw decoded string
          return atob(base64);
        }
      };
    ...
    <div className={`fixed inset-y-0 right-0 w-96 bg-white dark:bg-gray-900 shadow-lg transform ${
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold">JWT Token Tool</h2>
          <button
            onClick={onClose}
...
// Add KeyGenerator component after JwtDecoder component
const KeyGenerator = () => {
  ...
  const generateKeys = async () => {
    try {
      ...
      cert.setIssuer(attrs); // Self-signed certificate
  ...
  return (
    <div className="p-4 h-full overflow-y-auto">
      <h3 className="text-lg font-semibold mb-4 sticky top-0 bg-white dark:bg-gray-800 py-2">Generate JWT Keys</h3>
      
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium mb-1">Country Name (Two Letter Code)</label>
          <input
            type="text"
            name="countryName"
            value={certDetails.countryName}
        ...
        <div>
          <label className="block text-sm font-medium mb-1">State or Province</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Locality Name</label>
          <input
            type="text"
            name="localityName"
            value={certDetails.localityName}
        ...
        <div>
          <label className="block text-sm font-medium mb-1">Organizational Unit Name</label>
          <input
            type="text"
        ...
        {success && (
          <div className="mt-4">
            <h4 className="font-semibold mb-2">Generated Keys:</h4>
...
const App = () => {
  ...
  // Add this state variable to the App component
  const [showEnvironmentDropdown, setShowEnvironmentDropdown] = useState(false);

  // Load environments from localStorage when component mounts
  useEffect(() => {
    try {
      ...
      if (storedData) {
        ...
        if (parsedData.environments) {
          ...
          // Set active environment if it exists
...
const XMLView = ({ data }) => {
  const formatXML = (xml) => {
    ...
    if (typeof xml.data === 'object') {
      ...
      formatted += processObject(xml.data || {}, indent);
    } else if (xml.data !== undefined) {
      // Handle primitive data types
      formatted += `${getIndent(indent)}<content>${xml.data}</content>\n`;
    }
    ...
    if (xml.isBase64Encoded !== undefined) {
    ...
    switch (viewFormat) {
      ...
      case 'xml':
        ...
        try {
          ...
          if (typeof api.responseData.data === 'object') {
            textToCopy = xml.js2xml(api.responseData.data, { compact: true, spaces: 2 });
          } else if (typeof api.responseData.data === 'string') {
            // If it's already a string, use it directly
            textToCopy = api.responseData.data;
          } else {
            // Fallback
            textToCopy = String(api.responseData.data);
          }
        } catch (error) {
  ...
  />
   ...
   <RenameModal 
      ...
      {toast.show && (
        ...
        {!isMobile && (
          <ResizableBox
            width={isRightSidebarCollapsed ? 70 : rightSidebarWidth}
            height={Infinity}
            minConstraints={[70, Infinity]}
            ...
            className={`
              ...
              transform transition-transform duration-300
              ${isRightSidebarCollapsed ? 'w-[70px]' : ''}
            `}
          >
